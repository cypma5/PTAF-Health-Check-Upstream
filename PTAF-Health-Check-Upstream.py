...
id_upstream = "ID_Ipstream"

#Задаем переменную с URL по которому 
url_upstreams = "https://ip:8443/api/waf/v2/upstreams" + '/' + id_upstream

#Указываем заголовки
headers_ptaf = {'Authorization': 'Basic XXXXXXXXXXXXXXXXXXXXXXXXX'}
headers_contentType = {'Authorization':'Basic XXXXXXXXXXXXXXXXXXXXXXXXX' , 'Content-Type':'application/json'}
headers_upstream = {}
...
for n in JSON_data['addresses']:
    ...
    #url_healthcheck = 'http://' + str(JSON_data["backends"][count]["address"])
    ...
    print(now , url_healthcheck + ' healthceck status code ' + str(HealthCheck.status_code))
    if  HealthCheck.status_code == 200:
        ...
        count =  count + 1
            ...
            #Test_payload = '{"backends": [ {"address": "10.10.10.10","backup": "false","down": "false","max_fails": 1,"port": 443,"weight": 1},{"address": "10.10.10.20","backup": "false","down": "false","max_fails": 1,"port": 80,"weight": 1}]}'
            #print(now + ' Test_payload =' , Test_payload)
            payload_upstream = json.dumps(JSON_data["backends"])
