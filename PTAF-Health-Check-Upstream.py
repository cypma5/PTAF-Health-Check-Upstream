        ...
        if response_upstream.status_code == 200:
            ...
            with open( str(list_upstream), encoding = 'UTF-8') as file_upstream:
                ...
                for n in JSON_data['addresses']:    #Запускаем цикл проверки
                    ...
                    logging.debug('Проверка Апстрима: '+ str(JSON_data["backends"][count]["address"])+':'+ str(JSON_data["backends"][count]["port"]))
                    ...
                    if result_upstream == 0: #Если порт открыт то
                        ...
                        #Ошибки при выполнении хелсчека
                            ...
                            count =  count + 1                            
                            logging.debug('HTTP_Health_check upstream_protocol Отправка https трафика на порт ожидающий http')
                            ...
                            logging.debug('HTTP_Health_check проверь Headers которые отправляешь на апстрим ')
                            ...
                            logging.debug('HTTP_Health_check Check variable upstream_protocol Отправка https трафика на порт ожидающий http')
                            logging.critical(str(id_upstreams) + '|' + str(JSON_data["backends"][count]["address"])+':'+ str(JSON_data["backends"][count]["port"]) +'|ok|false|Отправка https трафика на порт ожидающий http')
